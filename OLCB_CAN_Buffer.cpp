...
void OLCB_CAN_Buffer::setProducerIdentified(OLCB_NodeID* source, OLCB_Event* eid, uint8_t active)
{
    init(source->alias);
    if(active == EVENT_STATE_ACTIVE)
        setMTI(MTI_PRODUCER_IDENTIFIED_VALID);
    else if(active == EVENT_STATE_INACTIVE)
        setMTI(MTI_PRODUCER_IDENTIFIED_INVALID);
    else
